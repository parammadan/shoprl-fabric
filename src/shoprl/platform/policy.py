...
class StaleTrajectory(Exception):
    """A trajectory was generated by a policy too far behind the current one."""


def staleness_gate(current_version: int, trajectory_version: int,
                   max_staleness: int = 0, mode: str = "warn") -> tuple[int, str | None]:
    """Check a trajectory's policy staleness against a bound. Returns
    (staleness, warning_or_None). mode='reject' raises StaleTrajectory when the
    bound is exceeded; mode='warn' returns a message; mode='off' never flags.
    On single-machine on-policy training staleness is 0 (nothing flagged) — the
    gate exists to catch decoupled/lagging rollout, which is where drift creeps
    in silently."""
    s = staleness(current_version, trajectory_version)
    if mode == "off" or s <= max_staleness:
        return s, None
    msg = (f"stale trajectory: generated by policy v{trajectory_version}, "
           f"current v{current_version} ({s} behind > max {max_staleness})")
    if mode == "reject":
        raise StaleTrajectory(msg)
    return s, msg
